...
- **Guarantee**: Edge cases either:
  ...
  2. Raise explicit exception (e.g., `InsufficientDataError`)
...
2. **Verify determinism** in critical applications with repeated runs
3. **Document assumption** when tie-breaking affects >5% of universe

### Data Quality
1. **Validate min_periods** matches your data availability
2. **Pre-filter** assets with insufficient history before preselection
3. **Monitor** NaN score rates in production

### Configuration
1. **Start conservative**: Use validated defaults (lookback=252, skip=1)
2. **Test edge values**: Verify behavior at boundaries before production
3. **Document rationale**: Record why specific config values chosen

### Production Deployment
1. **Monitor factor distributions** to detect data issues
2. **Log selection changes** between periods for audit trail
3. **Test with synthetic edge cases** matching your data characteristics
4. **Implement alerting** for unexpected empty selections
