...
1. **Preselection** - Factor-based asset filtering
1. **Membership Policy** - Control portfolio turnover
...
1. **Rank buffer**: Existing holdings within buffer rank are kept (even if outside top-K)
1. **Max changes**: Limit additions and removals
1. **Turnover cap**: Limit total portfolio value that can change
...
1. **Universe YAML** (middle) - Defined in universe configuration
1. **Defaults** (lowest) - Built-in default values
...
1. Don't set CLI flags for these features
...
1. Preselection not providing ranks: Membership policy needs ranked candidates
1. Policy parameters too loose: Tighten constraints (lower buffer_rank, max_turnover, etc.)
