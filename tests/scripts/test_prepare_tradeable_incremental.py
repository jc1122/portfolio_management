...
def test_incremental_resume_skips_when_unchanged(tmp_path: Path) -> None:
    ...
    cmd1 = [
        ...
        "--metadata-output", str(index_path),
        "--tradeable-dir", str(tradeable_dir),
        "--match-report", str(match_report),
        "--unmatched-report", str(unmatched_report),
        "--prices-output", str(prices_dir),
        "--incremental",
        "--cache-metadata", str(cache_file),
        "--log-level", "INFO",
    ]
    ...
    assert "incremental resume" in result2.stdout.lower() or "incremental resume" in result2.stderr.lower()
...
def test_incremental_resume_reruns_when_tradeable_changed(tmp_path: Path) -> None:
    ...
    assert "changed" in output.lower() or "running" in output.lower() or "notimplementederror" in output.lower()
